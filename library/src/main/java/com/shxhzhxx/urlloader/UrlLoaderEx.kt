...
class UrlLoaderEx(cachePath: File, @IntRange(from = 1) maxCacheSize: Int = 100 * 1024 * 1024, cacheSeed: String = cachePath.absolutePath) : TaskManagerEx<CallbackEx, File>() {
    ...
    ) = asyncStart(url, { Worker(url) }, tag, CallbackEx(onComplete, onFailed, onCanceled, onProgress)).also { id ->
        if (id < 0) {
            onFailed?.invoke()
        }
    }
    ...
    fun syncLoad(url: String, canceled: () -> Boolean, onProgress: ((total: Long, current: Long, speed: Long) -> Unit)? = null) =
            syncStart(url, { Worker(url) }, canceled, CallbackEx(onProgress = onProgress))
    ...
    private inner class Worker(private val url: String) : Task(url) {
        ...
        override fun onObserverUnregistered(observer: CallbackEx?) {
